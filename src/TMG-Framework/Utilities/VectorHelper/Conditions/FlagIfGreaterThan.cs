...
public static partial class VectorHelper
{
    ...
    public static void FlagIfGreaterThan(Span<float> dest, float lhs, ReadOnlySpan<float> rhs)
    {
        ...
        if (Vector512.IsHardwareAccelerated && length >= (nuint)Vector512<float>.Count)
        {
            ...
            for (; i <= length - (nuint)Vector512<float>.Count; i += (nuint)Vector512<float>.Count)
            {
                ...
                var destV = Blend(vZero, vOne, Vector512.GreaterThan(lhsV, rhsV));
            ...
            if (i <= length - (nuint)Vector256<float>.Count)
            {
                ...
                var destV = Blend(vZero256, vOne256, Vector256.GreaterThan(lhsV.GetLower(), rhsV));
                destV.StoreUnsafe(ref pDest, i);
        ...
        else if (Vector256.IsHardwareAccelerated && length >= (nuint)Vector256<float>.Count)
        {
            ...
            {
                var rhsV = Vector256.LoadUnsafe(ref pRhs, i);
                var destV = Blend(vZero, vOne, Vector256.GreaterThan(lhsV, rhsV));
                destV.StoreUnsafe(ref pDest, i);
            }
        }
    ...
    public static void FlagIfGreaterThan(Span<float> dest, ReadOnlySpan<float> lhs, float rhs)
    {
        ...
        if (Vector512.IsHardwareAccelerated && length >= (nuint)Vector512<float>.Count)
        {
            ...
            if (i <= length - (nuint)Vector256<float>.Count)
            {
                ...
                var destV = Blend(vZero256, vOne256, Vector256.GreaterThan(lhsV, rhsV.GetLower()));
                destV.StoreUnsafe(ref pDest, i);
                i += (nuint)Vector256<float>.Count;
            }
        }
    ...
    public static void FlagIfGreaterThan(Span<float> dest, ReadOnlySpan<float> lhs, ReadOnlySpan<float> rhs)
    {
        ...
        if (Vector512.IsHardwareAccelerated && length >= (nuint)Vector512<float>.Count)
        {
            ...
            if (i <= length - (nuint)Vector256<float>.Count)
            {
                ...
                var rhsV = Vector256.LoadUnsafe(ref pRhs, i);
                var lhsV = Vector256.LoadUnsafe(ref pLhs, i);
                var destV = Blend(vZero256, vOne256, Vector256.GreaterThan(lhsV, rhsV));
                destV.StoreUnsafe(ref pDest, i);
                i += (nuint)Vector256<float>.Count;
            }
        }
    ...
    /// </summary>
    public static void FlagIfGreaterThan(float[] dest, int destIndex, float lhs, float[] rhs, int rhsIndex, int length)
    {
        FlagIfGreaterThan(new Span<float>(dest, destIndex, length), lhs, new ReadOnlySpan<float>(rhs, rhsIndex, length));
    }

    /// <summary>
    /// Set the value to one if the condition is met.
    /// </summary>
    public static void FlagIfGreaterThan(float[] dest, int destIndex, float[] lhs, int lhsIndex, float rhs, int length)
    {
        FlagIfLessThan(dest, destIndex, rhs, lhs, lhsIndex, length);
    }

    /// <summary>
    /// Set the value to one if the condition is met.
    /// </summary>
    public static void FlagIfGreaterThan(float[] dest, int destIndex, float[] lhs, int lhsIndex, float[] rhs, int rhsIndex, int length)
    {
        FlagIfGreaterThan(new Span<float>(dest, destIndex, length), new ReadOnlySpan<float>(lhs, lhsIndex, length), new ReadOnlySpan<float>(rhs, rhsIndex, length));
    }
}
