...
public static partial class VectorHelper
{
    ...
    public static void FlagIfEquals(Span<float> dest, float lhs, ReadOnlySpan<float> rhs)
    {
        ...
        if (Vector512.IsHardwareAccelerated && length >= (nuint)Vector512<float>.Count)
        {
            ...
            for (; i <= length - (nuint)Vector512<float>.Count; i += (nuint)Vector512<float>.Count)
            {
                ...
                var destV = Blend(vZero, vOne, Vector512.Equals(rhsV, lhsV));
            ...
            if (i <= length - (nuint)Vector256<float>.Count)
            {
                ...
                var destV = Blend(vZero256, vOne256, Vector256.Equals(rhsV, lhsV.GetLower()));
                destV.StoreUnsafe(ref pDest, i);
        ...
        else if (Vector256.IsHardwareAccelerated && length >= (nuint)Vector256<float>.Count)
        {
            ...
            {
                var rhsV = Vector256.LoadUnsafe(ref pRhs, i);
                var destV = Blend(vZero, vOne, Vector256.Equals(rhsV, lhsV));
                destV.StoreUnsafe(ref pDest, i);
            }
        }
    ...
    /// <param name="lhs">The scalar value to compare against.</param>
    /// <param name="rhs">The data span.</param>
    public static void FlagIfEquals(Span<float> dest, ReadOnlySpan<float> lhs, ReadOnlySpan<float> rhs)
    {
        ...
        if (Vector512.IsHardwareAccelerated && length >= (nuint)Vector512<float>.Count)
        {
            ...
            if (i <= length - (nuint)Vector256<float>.Count)
            {
                ...
                var destV = Blend( vZero256, vOne256, Vector256.Equals(rhsV, lhsV));
                destV.StoreUnsafe(ref pDest, i);
                i += (nuint)Vector256<float>.Count;
            }
        }
        else if (Vector256.IsHardwareAccelerated && length >= (nuint)Vector256<float>.Count)
        {
            ...
            for (; i <= length - (nuint)Vector256<float>.Count; i += (nuint)Vector256<float>.Count)
            {
                ...
                var destV = Blend(vZero, vOne, Vector256.Equals(dataV, vValue));
                destV.StoreUnsafe(ref pDest, i);
            }
        }
        // Process the remainder.
    ...
    /// </summary>
    public static void FlagIfEquals(float[] dest, int destIndex, float lhs, float[] rhs, int rhsIndex, int length)
    {
        FlagIfEquals(new Span<float>(dest, destIndex, length), lhs, new ReadOnlySpan<float>(rhs, rhsIndex, length));
    }

    /// <summary>
    /// Set the value to one if the condition is met.
    /// </summary>
    public static void FlagIfEquals(float[] dest, int destIndex, float[] lhs, int lhsIndex, float rhs, int length)
    {
        FlagIfEquals(dest, destIndex, rhs, lhs, lhsIndex, length);
    }

    /// <summary>
    /// Set the value to one if the condition is met.
    /// </summary>
    public static void FlagIfEquals(float[] dest, int destIndex, float[] lhs, int lhsIndex, float[] rhs, int rhsIndex, int length)
    {
        FlagIfEquals(new Span<float>(dest, destIndex, length), new ReadOnlySpan<float>(lhs, lhsIndex, length), new ReadOnlySpan<float>(rhs, rhsIndex, length));        
    }
}
