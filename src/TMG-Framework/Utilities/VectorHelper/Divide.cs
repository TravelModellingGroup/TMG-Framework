...
public static partial class VectorHelper
{
    ...
    public static void Divide(Span<float> dest, ReadOnlySpan<float> left, ReadOnlySpan<float> right)
    {
        ...
        if (Vector512.IsHardwareAccelerated && dest.Length >= Vector512<float>.Count)
        {
            ...
            {
                var vLeft256 = Vector256.LoadUnsafe(ref leftRef, i);
    ...
    public static void Divide(Span<float> dest, float left, ReadOnlySpan<float> right)
    {
        EnsureSameSize(dest, right);
        nuint i = 0;
        var end = (nuint)dest.Length - 16;
    ...
    public static void Divide(Span<float> dest, ReadOnlySpan<float> left, float right)
    {
        ...
        for (; i < (nuint)dest.Length; i++)
        {
            Unsafe.Add(ref destRef, i) = Unsafe.Add(ref leftRef, i) / right;
        }
    }
