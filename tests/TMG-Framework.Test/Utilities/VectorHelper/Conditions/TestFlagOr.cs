...
[TestClass]
public class TestFlagOr
{
    [TestMethod]
    public void FlagOrVectorScalar()
    {
        ...
        TMG.Utilities.VectorHelper.FlagOr(dest, value, data);

        for (int i = 0; i < 100; i++)
        {
            float expected = (data[i] != 0f) ? 1.0f : 0f;
    ...
    [TestMethod]
    public void FlagOrVectorScalarWrongSize()
    {
    ...
    [TestMethod]
    public void FlagOrVectorScalarZero()
    {
        Span<float> dest = stackalloc float[100];
        Span<float> data = stackalloc float[100];
        var value = 0f;        
        ...
        for (int i = 0; i < 100; i++)
        {
            float expected = data[i] != 0.0f ? 1.0f : 0f;
            // Avoid calling out to AreEqual to help performance unless it failed.
            if (dest[i] != expected)
            {
                Assert.AreEqual(expected, dest[i]);
    ...
    [TestMethod]
    public void FlagOrVectorVector()
    {
        Span<float> dest = stackalloc float[100];
        Span<float> data = stackalloc float[100];
        ...
        for (int i = 0; i < 100; i++)
        {
            float expected = ((data[i] != 0f) || (value[i] != 0f)) ? 1.0f : 0f;
            // Avoid calling out to AreEqual to help performance unless it failed.
            if (dest[i] != expected)
            {
                Assert.AreEqual(expected, dest[i]);
    ...
    [TestMethod]
    public void FlagOrVectorVectorWrongSize()
    {
