        ...
        public static void FlagIfGreaterThanOrEqual(float[] dest, int destIndex, float lhs, float[] rhs, int rhsIndex, int length)
        {
            if (System.Numerics.Vector.IsHardwareAccelerated)
            {
                ...
                for (i = 0; i < length - Vector<float>.Count; i += Vector<float>.Count)
                {
                    ...
                    System.Numerics.Vector.ConditionalSelect(System.Numerics.Vector.GreaterThanOrEqual(vLhs, vRhs), one, zero).CopyTo(dest, i);
                }
                for (; i < length; i++)
                {
        ...
        public static void FlagIfGreaterThanOrEqual(float[] dest, int destIndex, float[] lhs, int lhsIndex, float rhs, int length)
        {
            if (System.Numerics.Vector.IsHardwareAccelerated)
            {
                ...
                for (i = 0; i < length - Vector<float>.Count; i += Vector<float>.Count)
                {
                    ...
                    System.Numerics.Vector.ConditionalSelect(System.Numerics.Vector.GreaterThanOrEqual(vLhs, vRhs), one, zero).CopyTo(dest, destIndex + i);
                }
                for (; i < length; i++)
                {
