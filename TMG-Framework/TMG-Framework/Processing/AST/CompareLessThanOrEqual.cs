        ...
        public override ComputationResult Evaluate(ComputationResult lhs, ComputationResult rhs)
        {
            ...
            if (lhs.IsValue)
            {
                if (rhs.IsVectorResult)
                {
                    ...
                    VectorHelper.FlagIfLessThanOrEqual(flat, lhs.LiteralValue, rhs.VectorData.Data);
                    return new ComputationResult(retVector, true);
                }
                else
                {
                    var retMatrix = rhs.Accumulator ? rhs.OdData : new SparseMatrix(rhs.OdData);
                    VectorHelper.FlagIfLessThanOrEqual(retMatrix.Data, lhs.LiteralValue, rhs.OdData.Data);
            ...
            else if (rhs.IsValue)
            {
                if (lhs.IsVectorResult)
                {
                    ...
                    var flat = retVector.Data;
                    VectorHelper.FlagIfLessThanOrEqual(flat, lhs.VectorData.Data, rhs.LiteralValue);
                    return new ComputationResult(retVector, true);
                }
                else
                {
                    ...
                    VectorHelper.FlagIfLessThanOrEqual(retMatrix.Data, lhs.OdData.Data, rhs.LiteralValue);
                    return new ComputationResult(retMatrix, true);
                }
            }
            else
            {
                if (lhs.IsVectorResult || rhs.IsVectorResult)
                {
                    if (lhs.IsVectorResult && rhs.IsVectorResult)
                    {
                        ...
                        VectorHelper.FlagIfLessThanOrEqual(retMatrix.Data, 0, lhs.VectorData.Data, 0, rhs.VectorData.Data, 0, retMatrix.Data.Length);
                ...
                else
                {
                    ...
                    VectorHelper.FlagIfLessThanOrEqual(retMatrix.Data, 0, lhs.OdData.Data, 0, rhs.OdData.Data, 0, lhs.OdData.Data.Length);
                    return new ComputationResult(retMatrix, true);
                }
            }
        }
