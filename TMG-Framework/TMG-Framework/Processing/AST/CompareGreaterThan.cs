        ...
        public override ComputationResult Evaluate(ComputationResult lhs, ComputationResult rhs)
        {
            ...
            if (lhs.IsValue)
            {
                if (rhs.IsVectorResult)
                {
                    ...
                    VectorHelper.FlagIfGreaterThan(flat, 0, lhs.LiteralValue, rhs.VectorData.Data, 0, rhs.VectorData.Data.Length);
                ...
                else
                {
                    ...
                    VectorHelper.FlagIfGreaterThan(retMatrix.Data, 0, lhs.LiteralValue, rhs.OdData.Data, 0, rhs.OdData.Data.Length);
            ...
            else if (rhs.IsValue)
            {
                if (lhs.IsVectorResult)
                {
                    ...
                    var flat = retVector.Data;
                    VectorHelper.FlagIfGreaterThan(flat, 0, lhs.VectorData.Data, 0, rhs.LiteralValue, flat.Length);
                ...
                else
                {
                    ...
                    VectorHelper.FlagIfGreaterThan(retMatrix.Data, 0, lhs.OdData.Data, 0, rhs.LiteralValue, lhs.OdData.Data.Length);
                    return new ComputationResult(retMatrix, true);
                }
            }
            else
            {
                if (lhs.IsVectorResult || rhs.IsVectorResult)
                {
                    if (lhs.IsVectorResult && rhs.IsVectorResult)
                    {
                        ...
                        VectorHelper.FlagIfGreaterThan(retMatrix.Data, 0, lhs.VectorData.Data, 0, rhs.VectorData.Data, 0, retMatrix.Data.Length);
                    ...
                    else if (lhs.IsVectorResult)
                    {
                        ...
                        if (lhs.Direction == ComputationResult.VectorDirection.Vertical)
                        {
                            ...
                            {
                                VectorHelper.FlagIfGreaterThan(retMatrix.Data, i * rowSize, flatLhs[i], flatRhs, i * rowSize, rowSize);
                            }
                        }
                        else if (lhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            for (int i = 0; i < rowSize; i++)
                            {
                                VectorHelper.FlagIfGreaterThan(retMatrix.Data, i * rowSize, flatLhs, 0, flatRhs, i * rowSize, rowSize);
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (rhs.Direction == ComputationResult.VectorDirection.Vertical)
                        {
                            for (int i = 0; i < rowSize; i++)
                            {
                                VectorHelper.FlagIfGreaterThan(retMatrix.Data, i * rowSize, flatLhs, i * rowSize, flatRhs[i], rowSize);
                            }
                        }
                        else if (rhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            for (int i = 0; i < rowSize; i++)
                            {
                                VectorHelper.FlagIfGreaterThan(retMatrix.Data, i * rowSize, flatLhs, i * rowSize, flatRhs, 0, rowSize);
                            }
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    VectorHelper.FlagIfGreaterThan(retMatrix.Data, 0, lhs.OdData.Data, 0, rhs.OdData.Data, 0, retMatrix.Data.Length);
                    return new ComputationResult(retMatrix, true);
                }
            }
        }
